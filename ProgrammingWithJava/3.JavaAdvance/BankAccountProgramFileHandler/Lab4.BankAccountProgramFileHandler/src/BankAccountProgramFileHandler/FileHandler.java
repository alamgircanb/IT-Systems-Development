    ...
    public BankAccount[] getData(String fileName) {
        try {
            ...
            for (int i = 0; i < numAccounts; i++) {
                ...
                char middleInit = input.next().charAt(0);//to indicate the first character
                float balance = input.nextFloat();
                Date lastTransaction = new Date(input.nextInt(),//year
                                                input.nextInt(),//Month
                                                input.nextInt());//Date
                // create an account
                Customer newCustomer= new Customer (customerId, firstName, lastName, middleInit);
                accounts[i] = new BankAccount(accountNum, newCustomer, balance, lastTransaction);
            }
        } 
